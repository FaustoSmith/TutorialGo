...
func main() {
	/*
						...
						strconv.ParseFloat(age,64)					//parsear a float un string,definimos si es floar de 32 o 64
...
func createBill() bill {
	var b bill
	reader := bufio.NewReader(os.Stdin)                   //leer lo que viene por la terminal
	name, _ := getInput("Create new bill name: ", reader) //el segundo parametro es el error que es el 2do argumento de retorno de la funcion ReadString
...
func propmptOptions(b bill) {
	...
	opt, _ := getInput("Select(a-add item ,s--save bill, t--addtip)", reader)
	switch opt {
	case "a":
		fmt.Println("a")
	case "s":
...
func (b *bill) save() {
	...
	data := []byte(b.format())
	os.WriteFile("bills/"+b.name+".txt", data, 0644) //Direccion,nombre delarchivo,arrgelo de bytes,codigo de permiso
